...
function renderCachedSearchResults(rawQuery = "") {
    ...
    const matchingPortals = searchablePortals().map((portal) => {
        ...
        return {portal: {...portal, recordMatches}, matchingHint, exactHint, hasMatch: Boolean(matchingHint) || recordMatches.length > 0};
...
function cachedSearchHasFileResult(cachedSearch = {}) {
    return Array.isArray(cachedSearch?.matchingPortals) && cachedSearch.matchingPortals.some(({portal}) => {
        return Array.isArray(portal?.recordMatches) && portal.recordMatches.some(match => match?.command === "files");
    });
}
...
document.querySelector(".status-indicator").popoutmenu([{icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" /></svg>`, label: "Refresh", action: () => location.reload()}, {icon: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>`, label: "Disconnect", destructive: true, action: () => window.location = "/logout"}]);
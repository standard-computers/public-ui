(() => {
	...
	const getCreateEventDateRange = () => {
		...
		} = getOrderedDateRange(selectedDateRangeStart || selectedDate, selectedDateRangeEnd || selectedDate);
	...
	const buildCalendarRangeControls = ({previous, next, label}) => div({
		...
		content: children([
			button({style: "naked tiny secondary",
	...
	const buildRecurrenceControls = (prefix, baseStart) => {
		...
		return div({
			style: "calendar-recurrence-shell", content: children([
				div({
					style: "calendar-recurrence-switch-row padded", content: children([
						div({style: "bold small-padding-top", content: "Reoccurring"}),
						switcher({id: `${prefix}-recurrence-enabled`, checked: false})
					])
				}),
				div({
					style: "calendar-recurrence-panel calendar-recurrence-panel-closed",
					id: `${prefix}-recurrence-panel`,
					content: children([
						div({style: "faded small-padding bold", content: "Reoccurrence pattern"}),
						div({
							style: "padded", content: select({
						...
						div({
							style: "calendar-recurrence-row padded", content: children([
								label({
									style: "calendar-recurrence-inline-label",
									input: `${prefix}-recurrence-interval`,
									content: "Recur every"
								}),
								input({
									id: `${prefix}-recurrence-interval`,
									type: "number",
									value: "1",
									style: "calendar-recurrence-number",
									placeholder: "1"
								}),
								div({
									style: "calendar-recurrence-unit",
									id: `${prefix}-recurrence-unit`,
									content: "week(s) on:"
								})
							])
						}),
						div({
							style: "calendar-recurrence-weekday-section",
							id: `${prefix}-recurrence-weekday-section`,
							content: buildWeekdaySelector(prefix, [selectedWeekday])
						}),
						div({
							style: "calendar-recurrence-monthly-section",
							id: `${prefix}-recurrence-monthly-section`,
							content: children([
								div({
									style: "calendar-recurrence-radio-row", content: children([
										label({
											style: "calendar-recurrence-radio-label", content: children([
												input({
													type: "radio",
													name: `${prefix}-recurrence-month-mode`,
													id: `${prefix}-recurrence-month-mode-day`,
													value: "day",
													checked: true
												}).replace("<input ", `<input data-recurrence-month-mode="${prefix}" `),
												"Day"
											])
										}),
										input({
											id: `${prefix}-recurrence-month-day`,
											type: "number",
											value: `${dayOfMonth}`,
											style: "calendar-recurrence-number",
								...
								div({
									style: "calendar-recurrence-radio-row", content: children([
										label({
											style: "calendar-recurrence-radio-label", content: children([
												input({
													type: "radio",
													name: `${prefix}-recurrence-month-mode`,
													id: `${prefix}-recurrence-month-mode-weekday`,
													value: "weekday"
												}).replace("<input ", `<input data-recurrence-month-mode="${prefix}" `),
												"The"
											])
										...
										select({
											id: `${prefix}-recurrence-month-ordinal`,
											value: "1",
											options: ORDINAL_OPTIONS
										}),
										select({
											id: `${prefix}-recurrence-month-weekday`,
						...
						div({
							style: "calendar-recurrence-yearly-section",
							id: `${prefix}-recurrence-yearly-section`,
							content: children([
								div({
									style: "calendar-recurrence-row", content: children([
										select({
											id: `${prefix}-recurrence-year-month`,
											value: `${startDate.getMonth() + 1}`,
											options: MONTH_OPTIONS
										}),
										input({
											id: `${prefix}-recurrence-year-day`,
											type: "number",
											value: `${dayOfMonth}`,
											style: "calendar-recurrence-number",
											placeholder: `${dayOfMonth}`
						...
						div({
							style: "calendar-recurrence-row padded", content: children([
								label({
									style: "calendar-recurrence-inline-label",
									input: `${prefix}-recurrence-occurrences`,
									content: "Occurrences"
								}),
								input({
									id: `${prefix}-recurrence-occurrences`,
									type: "number",
									value: "10",
									style: "calendar-recurrence-number",
									placeholder: "10"
	...
	const createEventFromPortal = portalIndex => {
		...
		Promise.all(occurrences.map(occurrence => createEventRecord({
			owner,
			category,
			name,
	...
	const saveSelectedEvent = portalIndex => {
		...
		Promise.all([
			...
			...extraOccurrences.map(occurrence => createEventRecord({
				owner,
				category,
				name,
				start: occurrence.start,
			...
			routes: [
				{
					...
					route: () => {
						...
						return div({
							style: "grid7 no-scrollbars larger-padding-top",
							id: "",
							content: () => CLI.send("[events]").then(eventsResponse => {
								const eventsByDate = {};
								...
								for (let i = 0; i < daysInYear; i++) {
									...
									ch.push(div({
										...
										content: children([
											div({
												style: "center strong small-padding tiny-text text-primary",
												content: tileDate.toLocaleString('default', {month: 'short'})
											}),
											div({
												style: "align-top", content: children([
													div({
														style: "float-right faded small-padding-right align-top",
														content: children([
															div({
																style: "super-tiny-text inline small-padding-right align-top",
																content: dayOfYear
															}),
															div({
																style: "tiny-text inline small-padding-right align-top",
																content: weekdayLabel
															}),
														])
													}),
													div({
														style: "strong inline small-padding-left faded align-top",
														content: tileDate.getDate()
													}),
												])
											}),
											div({
												style: "small-padding-left small-padding-right small-padding-bottom",
												content: children(tileEvents.map(eventEntry => {
													const event = eventEntry?.event || {};
													...
													const previewId = `calendar-event-chip-${calendarEventPreviewIndex++}`;
													calendarEventPreviewEntries.set(previewId, {
														eventEntry,
														date: tileDate
													});
													return div({
					...
					route: () => {
						...
						return div({
							style: "no-scrollbars large-padding-top",
							content: () => CLI.send("[events]").then(eventsResponse => {
				...
				style: "large-padding-top", content: children([
					div({
						style: "notes-list", content: div({
							style: "padded", content: () => CLI.send("[cats]").then(d => {
								...
								return children(categories.map(category => {
									...
									return div({
										style: "padded secondary-tile brick line small-spaced hover-shadowed pointer",
										content: children([
											div({
												style: "inline round small-icon space-right float-left margin-right",
												content: "",
												background: categoryColor
											}),
											div({
			...
			route: () => div({style: "large-padding-top", content: children([
					...
					div({
						content: children([
							...
							div({
								style: "padding-left padding-right", content: children([
									colorPicker({id: "calendar-category-color-picker", colors: modular.colors}),
									div({
										style: "small-padding margin-top", content: children([
											div({
												style: "inline round small-icon space-right",
												id: "calendar-category-color-preview",
												background: "#7B61FF"
											}),
											label({
												style: "inline tiny-text faded align-top very-small-padding-top",
												id: "calendar-category-color-value",
												content: "#7B61FF"
											})
										])
									})
					...
					style: "large-padding-top", content: children([
						div({
							content: children([
								...
								div({
									style: "padded",
									content: input({
										id: "edit-calendar-category-name",
										style: "undecorated no-padding",
										placeholder: "Work",
						...
						div({
							content: children([
								...
								input({
									type: "hidden",
									id: "edit-calendar-category-color",
									value: selectedCategory?.color || "#7B61FF"
								}),
								div({
									style: "padding-left padding-right", content: children([
										colorPicker({
											id: "edit-calendar-category-color-picker",
											colors: modular.colors
										}),
										div({
											style: "small-padding margin-top", content: children([
												div({
													style: "inline round small-icon space-right",
													id: "edit-calendar-category-color-preview",
													background: selectedCategory?.color || "#7B61FF"
												}),
												label({
													style: "inline tiny-text faded align-top very-small-padding-top",
													id: "edit-calendar-category-color-value",
													content: selectedCategory?.color || "#7B61FF"
												})
											])
										})
									])
			...
			route: () => div({
				style: "no-scrollbars large-padding-top", id: "", content: () => CLI.send("[cats]").then(response => {
					...
					const categoryOptions = categories.map(category => ({
						value: normalizeRecordId(category?.id),
						...
						return children([
							...
							div({
								content: children([
									...
									div({
										style: "padded", content: searchbox({

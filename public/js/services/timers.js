(() => {
	...
	const formatCreatedAt = timestamp => {
		...
		return Number.isNaN(date.getTime()) ? "" : `Created ${date.toLocaleTimeString([], {
			hour: "numeric",
			minute: "2-digit"
		})}`;
	};
		...
		timers = timers.flatMap(timer => {
			...
			if (action === "add-minute") {
				...
				return [{
					...
					status: "running",
					deadlineAt: currentTime + remainingMs,
					remainingMs,
					notifiedAt: 0
				}];
			}
	...
	window.StandardTimers = window.StandardTimers || {
		create: createTimer,
		createFromDescription: createTimerFromDescription,
		update: updateTimer,
		open: openTimers
	};

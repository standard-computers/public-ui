(async () => {
	...
	const applyBoardData = async (rawPath = "", boardData = {}, sourceNode = null) => {
		...
		boardViewport = {
			...
			translate: {
				x: Number(boardData?.viewport?.translate?.x) || 0,
				y: Number(boardData?.viewport?.translate?.y) || 0
			}
		};
	...
	const openBoardPath = async (rawPath = "", sourceNode = null) => {
		...
		try {
			...
			updateFileOpenProgress({
				label: `Opening ${download.fileName}`,
				loaded: download.blob.size || 1,
				total: download.blob.size || 1,
				indeterminate: false,
	...
	const saveBoardToBoardsFolder = async (boardName, canvas = null) => {
		...
		const response = await window.StandardUploads.saveFile(serializedBoard, `Boards/${fileName}`, {label: `Saving ${fileName}`});
		if (!response?.ok) throw new Error(`Unable to save board (${response?.status || 0})`);
		return {fileName, byteCount: response.byteCount, response: response.responseText || ""};
	};
	...
	const triggerBoardSave = sourceNode => {
		...
		if (!activeBoardName) {
			inputDialogue({
				title: "Board name", placeholder: "e.g. roadmap_q1", confirmation: (_, boardTitle) => {
	...
	const renderNoBoardsState = () => emptyState({
		style: "boards-empty-state",
		icon: "/icons/interfaces/whiteboard.png",
		iconStyle: "boards-empty-icon",
		label: "No boards",
		labelStyle: "boards-empty-label"
	});
			...
			routes: [
				{
						...
						style: "relative hidden large-padding-top", id: "boards-editor-root", content: children([
							...
							div({
								...
								content: children([
									...
									div({
										style: "color-block pointer hover-zoom cube inline background-white radius",
										altsync: "1",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-red radius",
										altsync: "2",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-orange radius",
										altsync: "3",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-yellow radius",
										altsync: "4",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-green radius",
										altsync: "5",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-blue radius",
										altsync: "6",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-indigo radius",
										altsync: "7",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-violet radius",
										altsync: "8",
										content: div({style: ""})
									}),
									div({
										style: "color-block pointer hover-zoom cube inline background-pink radius",
										altsync: "9",
										content: div({style: ""})
									}),
									button({
									...
									button({
										...
										altsync: "L",
										data: "tool-line",
										icon: `<svg class="small-icon text-color" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 19.5 19.5 4.5"/></svg>`
									}),
									button({
										style: "undecorated hover-background inner-radius float-right no-padding small-space-right adjust-top tool-button",
										altsync: "H",
					...
					afterRender: (windowBody) => {
						...
						const getViewportSize = () => {
							...
							return {
								width: Math.max(1, Math.round(containerRect?.width || 770)),
								height: Math.max(1, Math.round(containerRect?.height || 432))
							};
						};
						...
						const renderOperations = () => {
							...
							if (isSelectableOperation(selectedOperation)) {
								...
								const handles = [
									...
									{
										x: bounds.x + bounds.width - canvasOrigin.x,
										y: bounds.y + bounds.height - canvasOrigin.y
									}
								];
						...
						const getPos = e => {
							...
							return {
								x: ((e.clientX - rect.left) / scale) + canvasOrigin.x,
								y: ((e.clientY - rect.top) / scale) + canvasOrigin.y
							};
						};
						...
						const changeShapeProperty = (property, title, placeholder, parser = value => value) => {
							...
							inputDialogue({
								title, placeholder, value: shape[property], confirmation: (_, rawValue) => {
						...
						const startDrawing = e => {
							...
							currentStroke = {
								...
								color: pickedColor,
								width: tool === "erase" ? eraserWidth : 2,
								points: tool === "line" ? [pos, pos] : [pos]
							};

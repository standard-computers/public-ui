(() => {
	...
	const renderPortal = (windowNode, routeContext, snapshot) => {
		...
		currentIconNode.innerHTML = current?.icon ? img({
			src: current.icon,
			style: "large-icon contained float-left space-right"
		}) : "";
		...
		hourlyListNode.innerHTML = hourlyCards.length ? children(hourlyCards) : div({
